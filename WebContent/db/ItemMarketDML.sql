...
UPDATE MEMBER 
        ...
        WHERE (SELECT COUNT(TNO) FROM TRADE WHERE SID = 'bbb' OR BID='bbb') BETWEEN LOW AND HIGH AND MID='bbb') 
        WHERE MID = 'bbb';
...
-----------BOARD
-- 등록된 판매글수
SELECT COUNT(*) FROM BOARD WHERE BSELL='S';
-- 등록된 구매글수
SELECT COUNT(*) FROM BOARD WHERE BBUY='B';
--판매글목록
SELECT * FROM
